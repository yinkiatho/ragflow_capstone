...
class AttackSynthesizer:
    ...
    def generate_attacks(
        ...
        ignore_errors: bool,
        base_attacks_synthetic: list = None, #########################
    ) -> List[Attack]:
        ...
        for vulnerability in pbar:
            if base_attacks_synthetic:
                filtered_base_attacks = [i for i in base_attacks_synthetic if i.get('Vulnerability') == vulnerability.get_name()]
            else:
                filtered_base_attacks = []
            ...
            base_attacks.extend(
                self.generate_base_attacks(
                    ...
                    base_attacks_synthetic=filtered_base_attacks #####################
                )
            )
    ...
    def generate_base_attacks(
        ...
        if base_attacks_synthetic:
            for attack_dict in base_attacks_synthetic:
                base_attacks.append(
                    Attack(
                            vulnerability=attack_dict.get('Vulnerability'),
                            vulnerability_type=attack_dict.get('Vulnerability Type Enum'),
                            input=attack_dict.get('Base Input'),
                        )
                )
        ...
        for vulnerability_type in vulnerability.get_types():
            try:
                ...
                print(f"Generated Base attack for Vulnerability: {vulnerability_type}, {remote_attacks}")
                base_attacks.extend(
                    [
                        Attack(
