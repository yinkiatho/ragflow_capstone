...
async def run_test():
    ...
    vulnerabilities = [
                        ...
                        PIILeakage(types=[PIILeakageType.DATABASE_ACCESS, PIILeakageType.SESSION_LEAK, PIILeakageType.SOCIAL_MANIPULATION]),
                        PromptLeakage(types=[PromptLeakageType.GUARDS, PromptLeakageType.INSTRUCTIONS, PromptLeakageType.PERMISSIONS_AND_ROLES, PromptLeakageType.SECRETS_AND_CREDENTIALS]),
                        UnauthorizedAccess(types=[UnauthorizedAccessType.RBAC, UnauthorizedAccessType.SHELL_INJECTION, UnauthorizedAccessType.SQL_INJECTION])
    ...
    results = red_teamer.scan(
        target_model_callback=rag_agent.target_model_callback(guardrails=guardrails),
